<template>
  <div class="container">
    <form @submit.prevent="handleSubmit" @reset="handleReset" class="form-card">
      <h2 class="form-title">Edit Daftar Transaksi</h2>
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      const transaksi = existingTransaksi[id]
      
      if (transaksi) {
        form.kategori = transaksi.kategori
        form.jenis = transaksi.jenis
        form.tanggal = transaksi.tanggal
        form.nominal = transaksi.nominal
        form.keterangan = transaksi.keterangan
      } else {
