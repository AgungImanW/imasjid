<template>
  <div class="container">
    <form @submit.prevent="handleSubmit" @reset="handleReset" class="form-card">
      <h2 class="form-title">Edit Inventaris</h2>
...
export default {
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      const inventoryList = JSON.parse(localStorage.getItem('inventarisList') || '[]')
      const inventory = inventoryList[inventoryId]

      if (inventory) {
        form.kategori = inventory.kategori
        form.nama = inventory.nama
        form.jumlah = inventory.jumlah
        form.lokasi = inventory.lokasi
      } else {
    ...
    const handleSubmit = async () => {
      ...
      if (inventoryList[inventoryId]) {
        inventoryList[inventoryId] = {
          foto: fotoBase64,
          kategori: form.kategori,
          nama: form.nama,
          jumlah: parseInt(form.jumlah),
          lokasi: form.lokasi,
          tanggal_ditambahkan: inventoryList[inventoryId].tanggal_ditambahkan
        }
        
        localStorage.setItem('inventarisList', JSON.stringify(inventoryList))
