    ...
    <div class="dashboard" id="dashboard-container">
      <header class="header">
        ...
        <nav class="nav">
          ...
          <router-link to="/inventaris" class="nav-link" :class="{ 'active': $route.path === '/inventaris' }">Inventaris</router-link>
...
export default {
  ...
  methods: {
    ...
    addInventory() {
      this.$router.push('/inventaris/tambah');
    },
    editInventory(index) {
      this.$router.push(`/inventaris/edit/${index}`);
      console.log('Navigating to edit page for inventory:', this.inventoryList[index]);
    },
    addDonor() {
      this.$router.push('/inventaris/penyumbang/tambah');
    },
    editDonor(index) {
      const donorList = JSON.parse(localStorage.getItem('donorList') || '[]');
      if (donorList[index]) {
        this.$router.push(`/inventaris/penyumbang/edit/${index}`);
    ...
    addMovement() {
      this.$router.push('/inventaris/keluar-masuk/tambah')
    },
    editMovement(index) {
      this.$router.push(`/inventaris/keluar-masuk/edit/${index}`)
    },
    toggleStatus(index) {
